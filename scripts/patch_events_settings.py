#!/usr/bin/env python3
"""Дополнить settings.yaml секцией events."""
...
def main() -> int:
    ...
    if "events" not in data or not data.get("events"):
        data["events"] = ex.get("events", {})
        changed = True
        print("-> добавлена секция events из settings.example.yaml")
    elif isinstance(data.get("events"), dict) and not data["events"].get("sources"):
        data["events"] = {**(ex.get("events") or {}), **data["events"]}
        changed = True
        print("-> дополнена секция events из settings.example.yaml")
